...
@objcMembers final class AccountDetails: NSObject {
    enum AuthType: String {
        ...
        case implicitOauth = "http://opds-spec.org/auth/implicit-oauth2"
        case none
    }
    ...
    let userProfileLogInUrl:String?
    let userProfileLogOutUrl:String?
    let signUpUrl:URL?
    let loansUrl:URL?
    ...
    var needsAuth:Bool {
        return authType == .basic || authType == .implicitOauth
    }
    ...
    init(authenticationDocument: OPDS2AuthenticationDocument, uuid: String) {
        ...
        userProfileLogInUrl = authenticationDocument.links?.first(where: { $0.rel == "http://librarysimplified.org/terms/rel/user-profile-login" })?.href
        userProfileLogOutUrl = authenticationDocument.links?.first(where: { $0.rel == "http://librarysimplified.org/terms/rel/user-profile-logout" })?.href
...
@objcMembers final class Account: NSObject
{
    ...
    func loadAuthenticationDocument(completion: @escaping (Bool) -> ()) {
        guard let urlString = authenticationDocumentUrl, let url = URL(string: urlString) else {
            print("Invalid or missing authentication document URL")
            completion(false)
            return
        }

        // local load:
        let fileURL = Bundle.main.url(forResource:"authentication_document", withExtension: "json")! // szyjson does it load
        do {
            let fileData = try Data(contentsOf: fileURL)
            self.authenticationDocument = try OPDS2AuthenticationDocument.fromData(fileData)
            completion(true)
        } catch (let error) {
            print("""
                Failed to parse authentication document data for URL \(fileURL). Error:
...
@objc enum LoginKeyboard: Int {
    ...
    init?(_ stringValue: String?) {
        if stringValue == "Default" {
            ...
            print("Invalid init parameter for PatronPINKeyboard: \(stringValue ?? "nil")")
            return nil
        }
    }
}
